...
export function ItemEditor(props: ItemEditorProps) {
  ...
  const [currentView, setCurrentViewInternal] = React.useState<string | null>(
  ...
  const [detailViewActions, setDetailViewActions] = React.useState<
    RibbonAction[]
  >([]);
  ...
  const handleSetDetailViewActions = React.useCallback(
  ...
  const isDetailView = React.useMemo(() => {
    const view = resolvedViews.find(
      (v: RegisteredView) => v.name === currentView,
    );
  ...
  const notificationContent = React.useMemo(() => {
    ...
    if (Array.isArray(notifications)) {
      ...
      const activeNotifications = notifications.filter((notification) => {
        ...
        if (
          !notification.showInViews ||
          notification.showInViews.length === 0
        ) {
          return true;
        }
      ...
      return activeNotifications.length > 0
        ? activeNotifications[0].component
        : null;
    }
  ...
  const content = React.useMemo(() => {
    ...
    const activeView = resolvedViews.find(
      (v: RegisteredView) => v.name === currentView,
    );
  ...
  return (
    <div
      className={`item-editor-container ${className}`.trim()}
      data-testid="item-editor"
    >
      {/* Fixed ribbon at the top */}
      <div
        className="item-editor-container__ribbon"
        data-testid="item-editor-ribbon"
      >
        {ribbonContent}
      </div>
      ...
      {notificationContent && (
        <div
          className="item-editor-container__notification"
          data-testid="item-editor-notification"
        >
          {notificationContent}
        </div>
      )}
      ...
      {/* Scrollable content area */}
      <div
        className={`item-editor-container__content ${contentClassName}`.trim()}
        data-testid="item-editor-content"
      >
        <ViewNavigationContext.Provider value={navigationContext}>
          <DetailViewActionsContext.Provider
            value={{ setDetailViewActions: handleSetDetailViewActions }}
          >
            {content}
          </DetailViewActionsContext.Provider>
