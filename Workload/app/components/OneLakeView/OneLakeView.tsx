...
import {
  callDatahubOpen,
  callDatahubWizardOpen,
} from "../../controller/DataHubController";
...
export function OneLakeView(props: OneLakeViewProps) {
  ...
  const loadItemData = async (): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "OneLakeView: Failed to load data for item:",
          selectedItem,
          secondException,
        );
        success = false;
  ...
  useEffect(() => {
    if (selectedItem && selectedItem.id && selectedItem.workspaceId) {
      ...
      console.error(
        "OneLakeView: selectedItem is missing required properties:",
        selectedItem,
      );
  ...
  async function setTablesAndFiles(
    additionalScopesToConsent: string,
  ): Promise<boolean> {
    try {
      if (!selectedItem || !selectedItem.workspaceId || !selectedItem.id) {
        console.error(
          "OneLakeView: Cannot fetch data - selectedItem is invalid:",
          selectedItem,
        );
        return false;
      }
      ...
      console.log(
        `Fetching tables and files for item: ${selectedItem.id} in workspace: ${selectedItem.workspaceId}`,
      );
  ...
  function tableSelectedCallback(tableSelected: TableMetadata) {
    const tableFilePath = OneLakeStorageClient.getPath(
      selectedItem.workspaceId,
      selectedItem.id,
      tableSelected.relativePath,
    );
  ...
  async function fileSelectedCallback(fileSelected: FileMetadata) {
    const fullFilePath = OneLakeStorageClient.getPath(
      selectedItem.workspaceId,
      selectedItem.id,
  ...
  async function deleteFileCallback(filePath: string) {
    ...
    const clickedButton = await callDialogOpenMsgBox(
      ...
      t("OneLakeView_DeleteFile_Message", {
        fileName,
        defaultValue: `Are you sure you want to delete the file "${fileName}"?\n\nThis action cannot be undone.`,
      }),
    ...
    try {
      ...
      await callNotificationOpen(
        ...
        t(
          "OneLakeView_DeleteFileFailed_Message",
  ...
  async function deleteShortcutCallback(shortcutPath: string) {
    ...
    const clickedButton = await callDialogOpenMsgBox(
      ...
      t("OneLakeView_DeleteShortcut_Message", {
        folderName,
        defaultValue: `Are you sure you want to delete the shortcut folder "${folderName}"?\n\nThis will remove the shortcut but not the original data.\nThis action cannot be undone.`,
      }),
      [t("OneLakeView_Yes", "Yes"), t("OneLakeView_No", "No")],
    );
    ...
    try {
      const fullFolderPath = OneLakeStorageClient.getPath(
        selectedItem.workspaceId,
        selectedItem.id,
        shortcutPath,
      );
      ...
      await callNotificationOpen(
        ...
        t(
          "OneLakeView_DeleteShortcutFailed_Message",
          "Failed to delete shortcut. Please try again.",
        ),
        NotificationType.Error,
      );
    }
  ...
  async function createFolderCallback(parentPath: string) {
    ...
    try {
      ...
      await callNotificationOpen(
        ...
        t("OneLakeView_CreateFolderSuccess_Message", {
          folderName: folderName.trim(),
          defaultValue: `Folder "${folderName.trim()}" created successfully.`,
        }),
      ...
      await callNotificationOpen(
        ...
        t(
          "OneLakeView_CreateFolderFailed_Message",
  ...
  async function createShortcutCallback(parentPath: string) {
    try {
      ...
      const targetItemAndPath = await callDatahubWizardOpen(
        ...
        t(
          "OneLakeView_CreateShortcut_Description",
      ...
      const shortcutName = targetItemAndPath.selectedPath
        ? targetItemAndPath.selectedPath.split("/").pop() ||
      ...
      console.log(
        `Created shortcut "${shortcutName}" to item: ${targetItemAndPath.displayName}, path: ${targetItemAndPath.selectedPath}`,
      );
      ...
      await callNotificationOpen(
        ...
        t("OneLakeView_CreateShortcutSuccess_Message", {
          shortcutName,
          defaultValue: `Shortcut "${shortcutName}" created successfully.`,
        }),
        NotificationType.Success,
      );
      ...
      await callNotificationOpen(
        ...
        t(
          "OneLakeView_CreateShortcutFailed_Message",
  ...
  function getDefaultItemTypes() {
    ...
    const itemTypes =
      process.env.ITEM_NAMES?.split(",").map(
        (item) => `${workloadName}.${item.trim()}`,
      ) || [];
  ...
  async function onDatahubClicked() {
    if (!props.callbacks?.onItemChanged) {
      console.warn(
        "OneLakeView: onItemChanged callback is required for item selection",
      );
      return;
    }
    ...
    const result = await callDatahubOpen(
      ...
      t(
        "OneLakeView_SelectOneLakeItem_Title",
    ...
    return (
      <div className="onelake-view__container">
        ...
        >
          ...
          <span className="add">
            {t("OneLakeView_SelectItem_Text", "Select an item")}
          </span>
          {props.config?.allowItemSelection && (
            <Tooltip
              content={t(
                "OneLakeView_OpenDatahub_Tooltip",
                "Open Datahub Explorer",
              )}
              relationship="label"
            >
              <Button
    ...
    return (
      <div className="onelake-view__container">
        <div className="onelake-view__error">
          <Subtitle2>
            {t("OneLakeView_ErrorLoadingData_Title", "Error loading data")}
          </Subtitle2>
          <p>
            {t(
              "OneLakeView_PermissionError_Message",
              "Do you have permission to view this Item?",
            )}
          </p>
        </div>
      </div>
