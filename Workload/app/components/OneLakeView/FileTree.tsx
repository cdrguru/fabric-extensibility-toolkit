...
export function FileTree(props: OneLakeViewFilesTreeProps) {
  ...
  const {
    allFilesInItem: allFilesInOneLake,
    selectedFilePath,
    onSelectFileCallback,
    onDeleteFileCallback,
    onDeleteFolderCallback: onDeleteShortcutCallback,
  ...
  const [shortcutContents, setShortcutContents] = useState<
    Map<string, FileMetadata[]>
  >(new Map());
  const [loadingShortcuts, setLoadingShortcuts] = useState<Set<string>>(
    new Set(),
  );
  ...
  const loadShortcutContents = async (shortcutPath: string) => {
    ...
    try {
      ...
      console.error(
        `Failed to load shortcut contents for ${shortcutPath}:`,
        error,
      );
    } finally {
  ...
  const renderTreeNode = (node: TreeNode): JSX.Element => {
    ...
    if (metadata.isDirectory) {
      ...
      const folderIcon = metadata.isShortcut ? (
        <FolderLink20Regular />
      ) : (
        <FolderRegular />
      );
      ...
      return (
        <TreeItem key={metadata.relativePath} itemType="branch">
              ...
              <Tooltip relationship="label" content={metadata.name}>
                <TreeItemLayout
                  ...
                  onClick={async (e) => {
                    ...
                    if (
                      metadata.isShortcut &&
                      !expandedShortcuts.has(metadata.relativePath)
                    ) {
                      e.stopPropagation();
          ...
          <Tree>{children.map((child) => renderTreeNode(child))}</Tree>
        </TreeItem>
      );
    } else {
      return (
        <TreeItem key={metadata.relativePath} itemType="leaf">
          <Menu
            open={openMenu === metadata.relativePath}
            onOpenChange={(e, data) => {
          ...
          >
            <MenuTrigger disableButtonEnhancement>
              <Tooltip relationship="label" content={metadata.name}>
                <TreeItemLayout
                  className={
                    selectedFilePath === metadata.relativePath ? "selected" : ""
                  }
      ...
      {fileTree.length > 0
        ...
        : // Show context menu option when Files folder is empty
          (onCreateFolderCallback || onCreateShortcutCallback) && (
            ...
            >
              {mode === "edit"
                ? t("Right-click or click here to create a folder or shortcut")
                : t("Files folder is empty")}
            </div>
          )}
    </>
  );
