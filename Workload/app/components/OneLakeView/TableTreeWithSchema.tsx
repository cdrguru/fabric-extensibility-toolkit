      ...
      {tablesInOneLakeGroupedBySchema &&
        Object.keys(tablesInOneLakeGroupedBySchema).map((schema) => (
          <TreeItem id={schema} key={schema} itemType="branch">
            <Tooltip relationship="label" content={schema}>
              <TreeItemLayout
                id={schema}
                iconBefore={<ArrowCircleDownSplitRegular />}
              >
                {schema}
              </TreeItemLayout>
            </Tooltip>
            <Tree selectionMode="single" size="medium">
              {tablesInOneLakeGroupedBySchema[schema].map((table) => (
                ...
                >
                  <Tooltip relationship="label" content={table.name}>
                    <TreeItemLayout
                      className={
                        selectedTablePath === table.relativePath
                          ? "selected"
                          : ""
                      }
