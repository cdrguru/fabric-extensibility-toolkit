...
export async function getTables(
  ...
  const oneLakeContainer = await oneLakeStorageClient.getPathMetadata(
    workspaceId,
    directory,
    true,
  );
...
function convertToTableMetadata(
  path: OneLakeStoragePathMetadata,
  deltaLogDirectory: string,
): TableMetadata {
...
function convertToFileMetadata(
  path: OneLakeStoragePathMetadata,
  directory: string,
) {
  const pathName = path.name;
...
export async function getFilesInPath(
  ...
  const files = (oneLakeContainer.paths || []).map((path) => {
    ...
    const prefix = directoryPath.startsWith("Files/")
      ? "Files"
      : directoryPath.startsWith("Tables/")
        ? "Tables"
