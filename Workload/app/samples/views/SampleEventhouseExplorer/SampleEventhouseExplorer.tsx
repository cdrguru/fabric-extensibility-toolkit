...
export function EventhouseExplorerComponent({ workloadClient }: PageProps) {
  ...
  const [selectedEventhouseItemMetadata, setSelectedEventhouseItemMetadata] =
    useState<EventhouseItemMetadata>(undefined);
  const [isDirtyEventhouse, setDirtyEventhouse] = useState<boolean>(false);
  const [selectedDatabaseForQuery, setSelectedDatabaseForQuery] =
    useState<string>(undefined);
  const [selectedQueryToExecute, setSelectedQueryToExecute] =
    useState<string>(undefined);
  const [queryClientRequestId, setQueryClientRequestId] = useState<
    string | undefined
  >(undefined);
  ...
  async function loadKqlDatabasesWhenEventhouseSelected() {
    console.log(
      `loadKqlDatabasesWhenEventhouseSelected: ${selectedEventhouse}`,
    );
    if (selectedEventhouse) {
    ...
    <span>
      ...
      <div className="section">
        ...
        <Field label="Eventhouse ID" orientation="horizontal" className="field">
          <Input
            size="small"
            placeholder="Eventhouse ID"
            value={selectedEventhouse ? selectedEventhouse.id : ""}
          />
        </Field>
      </div>
      ...
      <div className="section">
        <div style={{ marginTop: "10px" }}>
          <Field label="Database" orientation="horizontal" className="field">
            <Combobox
              placeholder=""
              value={selectedDatabaseForQuery ?? ""}
              onOptionSelect={(_, opt) =>
                setSelectedDatabaseForQuery(opt.optionValue)
              }
              size="small"
            >
              {(
                selectedEventhouseItemMetadata?.properties?.databasesItemIds ||
                []
              ).map((item, index) => (
        ...
        <Field label="Query Uri" orientation="horizontal" className="field">
          <Input
            ...
            value={
              selectedEventhouseItemMetadata?.properties
                ? selectedEventhouseItemMetadata?.properties.queryServiceUri
                : ""
            }
            style={{ width: "400px" }}
