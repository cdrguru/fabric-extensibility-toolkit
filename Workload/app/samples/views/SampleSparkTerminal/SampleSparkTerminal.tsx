...
export const SampleSparkTerminal: React.FC<SampleSparkTerminalProps> = ({
  ...
  const [currentWorkspaceId, setCurrentWorkspaceId] = useState<
    string | undefined
  >(initialWorkspaceId);
  const [currentLakehouseId, setCurrentLakehouseId] = useState<
    string | undefined
  >(initialLakehouseId);
  ...
  async function onDatahubClicked() {
    ...
    if (sessionId) {
      addSystemMessage(
        "Lakehouse changed. You'll need to start a new session with the selected lakehouse.",
      );
      setSessionId(null);
  ...
  const waitForSessionReady = async (sid: string) => {
    ...
    while (attempts < maxAttempts) {
      try {
        if (!currentWorkspaceId || !currentLakehouseId) {
          addSystemMessage(
            "Lakehouse selection changed. Please start a new session.",
          );
          return false;
        }

        const sessionInfo = await sparkClient.getSession(
          currentWorkspaceId,
          currentLakehouseId,
          sid,
        );
        ...
        if (sessionInfo.state === SessionState.IDLE) {
          ...
          addSystemMessage(
            `Session failed to initialize with state: ${sessionInfo.state}`,
          );
          return false;
        }
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (statementInfo.state === "available") {
          ...
          if (statementInfo.output && statementInfo.output.data) {
            ...
            if (statementInfo.output.data["text/plain"]) {
              ...
              output = JSON.stringify(
                statementInfo.output.data["application/json"],
                null,
                2,
              );
            } else {
          ...
          if (
            statementInfo.output &&
            statementInfo.output.data &&
            statementInfo.output.data["text/plain"]
          ) {
  ...
  const checkStatementStatus = async (
    ...
    try {
      return await sparkClient.getStatement(
        currentWorkspaceId,
        currentLakehouseId,
        sessionId,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div>
            {selectedItem ? (
              ...
              <Text size={200} italic>
                No lakehouse selected
              </Text>
            )}
          </div>
          <Tooltip content={"Select a Lakehouse"} relationship="label">
            <Button size="small" onClick={onDatahubClicked} appearance="subtle">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <div className={styles.connectionStatus}>
              <div
                className={`${styles.statusIndicator} ${getConnectionStatusClass()}`}
              />
