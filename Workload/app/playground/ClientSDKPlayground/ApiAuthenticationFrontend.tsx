  ...
  return (
    <span>
      ...
      <div className="description">
        ...
        please make sure to add your AAD frontend application configuration to
        your workload manifest or configure the DEV_AAD_CONFIG_FE_APPID property
        in the .env.dev file for dev mode:&#10;
      </div>
      ...
      <Field orientation="horizontal" className="description">
        {" "}
        {token
          ? JSON.stringify(jwt_decode(token), null, "\t")
          : acquireTokenError}{" "}
      </Field>

      <Divider alignContent="start">
        Call an API with the token generated below
      </Divider>
      <Field label="API endpoint:" orientation="horizontal" className="field">
        <Input
          size="medium"
          placeholder="An API endpoint (e.g. https://onelake.dfs.fabric.microsoft.com/<workspace>...)"
      ...
      <Field label="Http method" orientation="horizontal" className="field">
        <Combobox
          placeholder="method"
          onOptionSelect={(_, opt) => setHttpMethod(opt.optionValue)}
        >
        ...
        <Button
          ...
          onClick={() =>
            sendWorkloadServerRequest(
              ...
              httpMethod,
              requestBody,
            ).then((result) => setServerResponse(result))
          }
        >
          Call server's API
        </Button>
      </div>
      <Field label="Response:" orientation="horizontal" className="field">
        {" "}
        {serverResponse}{" "}
      </Field>
    </span>
...
function sendWorkloadServerRequest(
  url: string,
  token: string,
  httpMethod: string,
  requestBody?: string,
