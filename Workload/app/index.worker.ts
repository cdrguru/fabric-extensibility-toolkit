  ...
  workloadClient.action.onAction(async function ({ action, data }) {
    ...
    switch (action) {
      case "item.onCreationSuccess":
        ...
        const itemTypeName = createdItem.itemType.substring(
          createdItem.itemType.lastIndexOf(".") + 1,
        );
        path = `/${itemTypeName}Item-editor`;
        console.log(
          `Item created successfully, redirecting to ${path}/${createdItem.objectId}`,
        );
        await callPageOpen(
          workloadClient,
          sampleWorkloadName,
          `${path}/${createdItem.objectId}`,
        );
