...
export async function getWorkloadItem<T>(
  ...
  const getItemDefinitionResult = await callGetItemDefinition(
    workloadClient,
    itemObjectId,
  );
  const item = convertGetItemResultToWorkloadItem<T>(
    getItemResult,
    getItemDefinitionResult,
    defaultDefinition,
  );
...
export async function saveWorkloadItem<T>(
  workloadClient: WorkloadClientAPI,
  itemWithDefinition: ItemWithDefinition<T>,
): Promise<UpdateItemDefinitionResult> {
  ...
  const defaultDefinitionPart = createDefaultDefinitionPart(
    itemWithDefinition.definition,
  );
...
export async function callGetItemDefinition(
  ...
  try {
    const itemDefinition: GetItemDefinitionResult =
      await workloadClient.itemCrud.getItemDefinition({
        itemId: itemId,
      });
    console.log(
      "Successfully fetched item definition for item %s: %o",
      itemId,
      itemDefinition,
    );
...
export function convertGetItemResultToWorkloadItem<T>(
    ...
    try {
      ...
      for (const part of itemDefinitionResult.definition.parts) {
        if (part.path === ItemDefinitionPath.Default) {
          ...
          itemPlatformMetadata = itemPlatformPayload
            ? itemPlatformPayload.metadata
            : undefined;
        } else {
      ...
      console.error(
        `Failed parsing payload for item ${itemResult?.item.id}, itemDefinitionResult: ${itemDefinitionResult}`,
        payloadParseError,
      );
    }
  }
  ...
  return {
    ...
    displayName:
      itemPlatformMetadata?.displayName ?? itemResult?.item.displayName,
    description:
      itemPlatformMetadata?.description ?? itemResult?.item.description,
...
export function buildPublicAPIPayloadWithParts(
  ...
  const itemDefinitionParts: ItemDefinitionPart[] = parts.map(
 ...
 */
export function convertGetDefinitionResponseToItemDefinition(
  responseBody: string,
): GetItemDefinitionResult {
  ...
  try {
    ...
    if (
      !responseItemDefinition?.definition?.parts ||
      !Array.isArray(responseItemDefinition.definition.parts)
    ) {
      throw new Error(
        "Invalid response format: missing definition.parts array",
      );
    }
    itemDefinition = {
      definition: {
        format: undefined,
        parts: responseItemDefinition.definition.parts.map(
          (part: ItemDefinitionPart) => ({
            path: part.path,
    ...
    console.error(
      `Failed parsing item definition, responseBody: ${responseBody}`,
      itemDefParseError,
    );
  }
  return itemDefinition;
