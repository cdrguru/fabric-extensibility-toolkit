 ...
 */
export async function callSettingsGet(
  workloadClient: WorkloadClientAPI,
): Promise<WorkloadSettings> {
  return await workloadClient.settings.get();
}

export async function callLanguageGet(
  workloadClient: WorkloadClientAPI,
): Promise<string> {
...
export function settingsToView(settings: WorkloadSettings): string {
  return [
    ...
    `Host Origin: ${settings.workloadHostOrigin}`,
    `Current Language Locale: ${settings.currentLanguageLocale}`,
    `API URI: ${settings.apiUri}`,
  ].join("\r\n");
}
