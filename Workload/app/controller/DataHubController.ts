...
export async function callDatahubWizardOpen(
  ...
  workspaceNavigationEnabled: boolean = true,
): Promise<ItemAndPath> {
  ...
  const result: DatahubWizardDialogResult =
    await workloadClient.datahub.openDatahubWizardDialog(datahubWizardConfig);
...
export async function callDatahubOpen(
  ...
  const result: DatahubSelectorDialogResult =
    await workloadClient.datahub.openDialog(datahubConfig);
